...
class Scan: UIViewController, QRCodeReaderViewControllerDelegate {
    ...
    func reader(_ reader: QRCodeReaderViewController, didScanResult result: QRCodeReaderResult) {
        ...
        final += result.value.substring(with: result.value.characters.index(result.value.startIndex, offsetBy: 3)..<result.value.characters.index(result.value.startIndex, offsetBy: 8))+","
        }
        if(result.value.hasPrefix("GTPACK:")) {
            let qid = result.value.substring(with: result.value.characters.index(result.value.startIndex, offsetBy: 11)..<result.value.characters.index(result.value.startIndex, offsetBy: 15))
            var line:String = ""
